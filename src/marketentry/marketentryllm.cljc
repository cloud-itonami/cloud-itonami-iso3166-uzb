(ns marketentry.marketentryllm
  ...
  the Republic of Uzbekistan (UZB) public-sector market-entry
  compliance actor.
  ...
  cited), never a committed record or a real xarid.uzex.uz tender
  response / equivalent filing. Every output is censored downstream by
...
(defn- propose-submit
  ...
  :actuation/submit-filing` -- real-world xarid.uzex.uz tender
  response / equivalent filing submission."
  [db {:keys [subject]}]
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向け提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "stir-verified?=" (:stir-verified? e)
                        " engages-local-staff-under-probation?=" (:engages-local-staff-under-probation? e)
                        " probation-employee-category=" (:probation-employee-category e)
                        " probation-period-months=" (:probation-period-months e)
