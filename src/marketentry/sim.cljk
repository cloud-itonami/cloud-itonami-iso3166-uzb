...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (UZB, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up probation-period-exceeds-ceiling) ==")
    ...
    (println "== filing/submit eng-4 (declared general-category probation period is 4 months, above Labour Code Art.130's 3-month ceiling -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up STIR-unregistered) ==")
    ...
    (println "== filing/submit eng-5 (STIR registration unverified -> HARD hold) ==")
    (println (exec-op actor "t11" {:op :filing/submit :subject "eng-5"} operator))

    (println "== jurisdiction/assess eng-6 (sets up an IN-range senior-management probation period -- clean) ==")
    ...
    (println "== filing/submit eng-6 (declared senior-management probation period is 6 months, exactly WITHIN Labour Code Art.130's ceiling -- clean submit, still escalates for approval) ==")
