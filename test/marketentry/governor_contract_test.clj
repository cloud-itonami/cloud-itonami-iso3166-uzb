  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Toshkent Savdo MChJ"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Toshkent Savdo MChJ" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest probation-period-exceeds-ceiling-is-held-and-unoverridable
  (testing "declared general-category probation period (4 months) exceeds Labour Code Art.130's 3-month ceiling -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:probation-period-exceeds-ceiling} (-> (store/ledger db) last :basis)))
...
(deftest stir-record-unverified-is-held-and-unoverridable
  (testing "unverified STIR registration when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:stir-record-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))

(deftest probation-period-within-ceiling-is-not-gated-by-the-flagship-check
  (testing "eng-6 declares a senior-management probation period that IS within Labour Code Art.130's ceiling (exactly 6 months) -- the flagship check never fires for it"
    (let [[_db actor] (fresh)
